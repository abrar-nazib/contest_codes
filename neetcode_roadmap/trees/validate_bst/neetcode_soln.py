...
class Solution:
    def isValidBST(self, root: Optional[TreeNode]) -> bool:
        def is_valid(root, left_boundary, right_boundary):
            if not root:
                return True
            if not (root.val > left_boundary and root.val < right_boundary):
                return False

            return (is_valid(root.left, left_boundary, root.val) and is_valid(root.right, root.val, right_boundary))
